...
3. メモリーは100件。ファクトリープリセットが1～85番まで入っており、86～100番はエンプティ。
4. エフェクトの並び順は直列のみだが、並び順は変更できる。
5. 単三電池２個で駆動。電源ボタンはなく、INPUTにシールドを挿すと起動する。
6. 本体でエディットもできるが、専用アプリ（有料）「Handy Guitar Lab for MS-50G+」を使うと便利。ただし「iPhone/iPad」版しかない。
...
## 3. 各部位の働き
1. フットスイッチ：長押しでチューナー起動。ホーム画面では、表示されているエフェクターのON/OFF。
2. 上下スイッチ：ホーム画面では、パッチを切り変える。LIBRARY画面では、上下移動。
3. 左右スイッチ：ホーム画面では、パッチのなかのエフェクターを選択。LIBRARY画面では、左右移動。
4. ADDプッシュノブ（左端）：
5. DELETEプッシュノブ（左から２番目）：
6. LIBLARYプッシュノブ（右から２番目）：
7. MENUプッシュノブ（右端）：
...
## 5. Factory Presets（プリセット・パッチメモリー）(85)の一覧
